...
const addUser = (data) => {
    return new Promise((resolve, reject) => {
        ...
        const id = uuidv4();
        db.run(sql, [id, data.username, data.password, data.email], (err) => {
            if (err) reject(err);
            resolve({id: id, username: data.username, password: data.password, email: data.email});
        })
    })
}
...
export { addData, getAllData, getData, deleteData, updateData, addUser, getUser };